...
- `League` — (platform ["sleeper" for now], platform_league_id, name, season, format, num_teams, roster_positions [JSON, Sleeper's array verbatim], team_names [JSON, {roster_id: name}], rank_set_id [nullable FK to `RankSet`], created_at) — one of your real leagues, set up once. Points at a `RankSet` via a plain reference (never the other way around — a `RankSet` has no idea which leagues use it); `Draft` doesn't reference a `League` yet (that's Phase C of League setup)
...
- `Draft` — (platform ["manual" or "sleeper"], platform_draft_id [set for a Sleeper-synced draft, null for manual], season, format, num_teams, num_rounds, my_slot, created_at) — a local draft session, not yet tied to a `League` (planned for League setup's Phase C)
...
**League setup — Phase A (RankSets)** ✅ done · **Phase B (League model + Sleeper sync)** ✅ done · **Phase C (Draft-from-League)** — not started
...
*Planned (Phase C)*: add `Draft.league_id`; a "From a League" mode in `DraftSetupForm` that auto-fills format/num_teams/num_rounds and looks up the league's current draft via Sleeper (`league_id` → `draft_id`, already returned on the league object) instead of pasting a draft ID; `DraftPlayerPool` switches from `GET /ranks`'s temporary resolver to `GET /rank-sets/{id}/ranks` via the league's real `rank_set_id`; `DraftRosterPanel` uses the league's real `roster_positions` (with a new eligibility mapping for Sleeper's flex sub-types) instead of the hardcoded QB/RB/RB/WR/WR/TE/FLEX/K/DEF template; team names replace "Team N" board headers.
