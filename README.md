...
Current state: Phase 0 scaffold, Phase 1 (Sleeper player list ingestion), Phase 2 (name-matching/reconciliation tool), Phase 3's in-app path (drag-and-drop rankings builder), Phase 4 (Sleeper ADP ingestion), the players management panel with in-UI data sync, Phases 5+8 (the draft view — board, player pool, queue, roster), and Phase 6 (live Sleeper sync) all done. Phase 3's Sheet-upload path and Phase 7 (availability-at-next-pick) are what's left.
...
- `Draft` — (platform ["manual" or "sleeper"], platform_draft_id [set for a Sleeper-synced draft, null for manual], season, format, num_teams, num_rounds, my_slot, created_at) — a local draft session, not yet tied to a `League` (doesn't exist yet)
