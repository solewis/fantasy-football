...
Current state: Phase 0 scaffold, Phase 1 (Sleeper player list ingestion), Phase 2 (name-matching/reconciliation tool), Phase 3's in-app path (drag-and-drop rankings builder, upgraded to named multi-per-format rank sets), Phase 4 (Sleeper ADP ingestion), the players management panel with in-UI data sync, Phases 5+8 (the draft view — board, player pool, queue, roster), Phase 6 (live Sleeper sync), and League setup Phases A+B+C (rank sets + the `League` model with Sleeper league sync + wiring Draft creation to a saved League) all done. Phase 3's Sheet-upload path and Phase 7 (availability-at-next-pick) are what's left.

Four tabs in the app: **Players** (browse/sync the full player+ADP list), **Rankings** (build your own rank order), **Leagues** (set up your real leagues once, synced from Sleeper), **Draft** (run an actual draft — board, player pool, queue, roster).
...
**Draft-from-League** (Phase C; `Draft.league_id`/`Draft.team_names`, `create_draft_from_league` in `app/draft.py`, a third `'league'` mode in `DraftSetupForm`): creates a draft directly from a saved `League` instead of re-entering its settings or pasting a raw Sleeper draft ID.

- **Setup**: pick a saved league from a dropdown (format and team count shown inline) and your draft slot — `POST /drafts/league` looks up the league's own `draft_id` from Sleeper (`GET /v1/league/{id}` already returns it) and pulls season/team count/round count from that draft, the same way Phase 6's Sleeper-sync mode does. A league with no active draft yet surfaces as a normal form error rather than creating a bogus one.
