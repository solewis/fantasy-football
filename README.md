...
Current state: Phase 0 scaffold, Phase 1 (Sleeper player list ingestion), Phase 2 (name-matching/reconciliation tool), Phase 3's in-app path (drag-and-drop rankings builder, upgraded to named multi-per-format rank sets), Phase 4 (Sleeper ADP ingestion), the players management panel with in-UI data sync, Phases 5+8 (the draft view — board, player pool, queue, roster), Phase 6 (live Sleeper sync), League setup Phases A+B+C (rank sets + the `League` model with Sleeper league sync + wiring Draft creation to a saved League), and nesting Draft under League (see below) all done. Phase 3's Sheet-upload path and Phase 7 (availability-at-next-pick) are what's left.

Three tabs in the app: **Leagues** (the home screen — set up your real leagues once synced from Sleeper, and drill into one to manage it and run its draft), **Rankings** (build your own rank order), **Players** (browse/sync the full player+ADP list — largely a debugging/data-sync tool at this point, kept separate since it's platform-neutral data and Sleeper is still the only platform this app knows). Draft is deliberately **not** a tab of its own anymore — see "Draft nested under League" below.
...
**In-app rankings builder** (`app/ranks.py`, `app/api/ranks.py`, `RankSet`/`RankEntry` models, `RankingsPage`/`reorder.ts` frontend): the first of two planned paths to build your ranks — an in-app drag-and-drop builder, with a Sheet-based upload still deferred as the other. Pick a scoring format; if there's no rank set yet for that format, the page seeds itself from the current ADP order (reusing the players endpoint, not a separate "seed" endpoint) and shows a note that you're starting from ADP.

**Named, multi-per-format rank sets** (upgraded from an original one-set-per-format `MyRank` table): you can build more than one rank set for the same format (e.g. a "Main" set and a "Zero-RB experiment"), name/rename/delete them, and pick which one you're editing from a toolbar dropdown. **Save Ranks** sends the complete current order to `PUT /rank-sets/{id}/ranks`, which always fully replaces the selected set rather than diffing — a rank list only ever has one current order, so there's nothing to reconcile. This exists specifically so a `League` can point at (reference) exactly one rank set without forcing every league sharing a format to fight over the same list — a `RankSet` has no idea which leagues use it, and `AdpEntry` stays completely separate (ADP is generic per platform/season/format, shared by every rank set and league that happens to use that format). The draft player pool's ADP-fallback fetch still calls `GET /ranks` for a manual or non-league-linked draft (resolving to whichever rank set was created first for that format) — a draft created from a `League` (Phase C, below) instead reads its assigned rank set directly via `GET /rank-sets/{id}/ranks`, so more than one rank set per format is now actually choosable per league.

**League setup** (`app/league.py`, `app/api/league.py`, `app/ingest/sleeper_league.py`, `League` model, `src/features/leagues/`): set up each of your real leagues once — point at a Sleeper league ID and the app remembers its scoring format, roster shape, and team names, instead of re-entering them every draft. Delivered as three phases, all done: **Phase A** (`RankSet`/`RankEntry`, above), **Phase B** (the `League` model + Sleeper sync), **Phase C** (wiring `Draft` creation to a saved `League`, and using its real roster shape/team names/rank set — see below).

- **Setup flow**: paste a Sleeper league ID, click **Look Up** (`GET /leagues/lookup` — a read-only preview, nothing saved yet) to see the league's name/team count and a suggested scoring format, confirm or change the format, pick an existing rank set for that format or create a new one inline (seeded from ADP, reusing Phase A's `POST /rank-sets` — same inline-create component, `RankSetPicker`, used both here and per-league afterward), then **Add League** commits it (`POST /leagues`, which re-fetches from Sleeper itself rather than trusting anything the lookup step cached).
- **Format is suggested, never assumed**: Sleeper's league object has no direct "PPR/half-PPR/standard" label (that only exists on a *draft's* metadata, a different object) — the suggestion comes from `scoring_settings.rec` (points per reception: 0 → std, 0.5 → half_ppr, 1.0 → ppr). Any other value (2QB/dynasty leagues aren't reliably inferable this way) gets no suggestion, and the field is always left editable regardless.
- **Team names**: joins the league's `/rosters` (`roster_id`/`owner_id`) against its `/users` (`user_id`/`display_name`/`metadata.team_name`) endpoints, falling back to the owner's display name if they haven't set a custom team name, and to a generic "Team N" label for an unclaimed roster slot. Stored as a `{roster_id: name}` map on the league (a JSON column — the first one in this app, deliberately here since it's opaque and order-*independent*, unlike `roster_positions` below).
- **Roster shape**: Sleeper's own `roster_positions` array (e.g. `["QB","RB","RB","WR","WR","TE","FLEX","K","DEF","BN","BN","BN","BN","BN"]`) is stored verbatim as a second JSON column — order-preserving, never queried into, so JSON is the right fit here too. Not used anywhere yet (that's Phase C, replacing `DraftRosterPanel`'s hardcoded template).
- **Sync**: a **Sync from Sleeper** button per league re-fetches name/team count/roster shape/team names (`POST /leagues/{id}/sync`) without touching your format choice or rank-set assignment — those are your decisions, not Sleeper's, and re-syncing shouldn't silently undo them.
- **Rank set reassignment**: each league's rank set can be changed anytime via the same `RankSetPicker` shown at setup (`PATCH /leagues/{id}/rank-set`) — not locked in at creation.
- **Deleting a rank set a league points at**: since this app has no `PRAGMA foreign_keys=ON`, `delete_rank_set` (Phase A) explicitly nulls out `League.rank_set_id` for any league that pointed at it, rather than leaving a dangling reference.

**Draft-from-League** (Phase C; `Draft.league_id`/`Draft.team_names`, `create_draft_from_league` in `app/draft.py`): creates a draft directly from a saved `League` instead of re-entering its settings or pasting a raw Sleeper draft ID.

- **Setup**: `POST /drafts/league` looks up the league's own `draft_id` from Sleeper (`GET /v1/league/{id}` already returns it) and pulls season/team count/round count from that draft, the same way Phase 6's Sleeper-sync mode does. A league with no active draft yet surfaces as a normal form error rather than creating a bogus one. (Originally reached via a third `'league'` mode on `DraftSetupForm` — that mode was removed once Draft nested under League, below; `POST /drafts/league` itself is unchanged, just called from `LeagueDetailPage` now.)
- **Rank set and roster shape are never copied onto `Draft`** — deliberately derived live from `Draft.league` at `get_status()` time (`rank_set_id`, `roster_positions`), so changing a league's assigned rank set or re-syncing its roster shape later automatically reflects in any of its existing drafts with zero staleness.
- **Team names** are draft-slot-keyed (`Draft.team_names`, `{draft_slot: name}`), distinct from `League.team_names` (`{roster_id: name}`) — a league doesn't know draft-slot assignments until a specific draft exists, so a helper joins the draft's own `slot_to_roster_id` (from Sleeper, empty pre-draft) against the league's roster-keyed names. Refreshed on every sync alongside picks, so slot↔team assignments (not yet known before the draft actually starts) fill in once Sleeper populates them.
- **Roster panel goes fully dynamic**: `DraftRosterPanel` renders whatever starter slots a league's real `roster_positions` describe (via a new pure `rosterSlots.ts` module) instead of the previous hardcoded QB/RB/RB/WR/WR/TE/FLEX/K/DEF template — including Sleeper's flex sub-types (`SUPER_FLEX`, `WRRB_FLEX`, `REC_FLEX`), each with a segmented multi-letter badge. Manual (non-league) drafts keep the original hardcoded template as a fallback.
- **Player pool reads the exact assigned rank set**: `DraftPlayerPool` now takes an optional `rankSetId` and, when present, calls `GET /rank-sets/{id}/ranks` directly rather than the format-based `GET /ranks` resolver — this is the first real caller that lets you choose *which* rank set (when a format has more than one) drives a draft, closing the gap the rank-sets upgrade left open.

*Done (Phase C)*: 205 backend tests total (new: `create_draft_from_league` success/error paths, sync refreshing `team_names`, `get_status` field coverage, `slot_to_roster_id` parsing/defaulting, API-level league-draft endpoint tests) + 107 frontend tests total (new: `rosterSlots.ts` pure-function suite, roster panel SUPER_FLEX rendering, player-pool rank-set-routing, draft board real-team-name rendering, setup-form league mode/slot-validation), all passing, lint/typecheck/format clean on both sides.

**Draft nested under League** (a follow-on to League setup, once it landed): with `League` doing the real work of remembering a league's settings, a top-level **Draft** tab sitting next to **Leagues** stopped making sense — starting or resuming a real draft is now "go to Leagues, pick a league, its draft is right there," not "go to a separate tab and pick a league from a dropdown there." Draft is no longer a tab at all; it lives inside a specific league's detail view, with a de-emphasized fallback for drafts that aren't tied to a saved league (mock drafts, tracking someone else's draft).

- **Drill-down navigation**: `LeaguesSection` (`src/features/leagues/LeaguesSection.tsx`) is the Leagues tab's container — it owns the leagues list, an unfiltered `GET /drafts` fetch turned into a `{league_id: draft}` map (one request, no per-league fan-out), and which of three views is showing: the list (`LeaguesPage`), a specific league's detail (`LeagueDetailPage`), or the ad-hoc non-league draft path (the existing `DraftPage`, now reached via a "Start a draft without a league →" footer link instead of a tab). `LeaguesPage`'s cards are real `<button>`s showing a "Draft in progress · Round N, Pick M" badge when the map has an entry, and are otherwise unchanged (the "+ Add League" lookup→confirm→create flow stays exactly where it was).
- **`GET /drafts?league_id=`** (`list_drafts` in `app/draft.py`): a new lightweight list endpoint — deliberately on the `drafts` router (not `GET /leagues/{id}/drafts`) since the dependency direction in this codebase is `app/draft.py` → imports `League`, never the reverse, and a nested-under-leagues route would force that arrow to flip. One grouped query joins `Draft`/`DraftPick` for a pick count (no N+1 across drafts), and `next_pick_number`/`current_round` are computed server-side via the existing `pick_to_round_and_slot` — the frontend has no snake-order math of its own to duplicate that with.
- **`DraftRoom`** (`src/features/draft/DraftRoom.tsx`): the actual board/pool/queue/roster/polling logic, extracted out of the old all-in-one `DraftPage` so both the ad-hoc path and a league's detail page can mount it against a `draftId` they already have. Takes a `headerActions` slot (rendered inside the existing header-actions styling) so each caller supplies its own "leave" control — the ad-hoc path's confirm-gated "New Draft" (losing its `localStorage` pointer would orphan that draft) vs. a league's lossless "← back to League" (the draft stays discoverable via `GET /drafts?league_id=`, so there's nothing to lose) — and an `onUnavailable` callback fired only on a genuine 404 (not a transient failure), so a caller can recover without risking a duplicate-draft-creation trap. A new `ApiError` (extends `Error`, carries the real HTTP status) in `api/draft.ts` is what makes that 404-vs-anything-else distinction possible client-side.
- **`LeagueDetailPage`** (`src/features/leagues/LeagueDetailPage.tsx`): everything that used to live on each `LeaguesPage` card (rank-set picker, team-name chips, Sync-from-Sleeper, Delete) plus a Draft section — a `my_slot` input + Start Draft when the league has no draft yet (server-side `my_slot` range validation was added to `create_draft_from_league` at the same time, since the only check before this was client-side in the now-removed `DraftSetupForm` league mode), or a Round/Pick summary + Resume (mounts `DraftRoom`) + a two-click "Start over" once one exists. Deleting a league with an in-progress draft now says so in the confirm label, since the delete cascades to the draft (see below).
- **Cascade-delete on league removal**: since this app has no `PRAGMA foreign_keys=ON`, `delete_league` now explicitly deletes a league's `Draft`/`DraftPick`/`DraftQueueEntry` rows before deleting the league itself — mirroring the same gap Phase A hit with `RankSet`/`League.rank_set_id`, but the opposite resolution: a `RankSet` outlives a league that stops pointing at it (it's just a reference), while a league-created `Draft` has no life without its league (`get_status` derives its `rank_set_id`/`roster_positions` live from `Draft.league`), so it's treated as an owned child instead.
- **`DraftSetupForm` drops its third `'league'` mode**: back to just Manual entry / Sync from Sleeper — a league's own detail page is now the only way to create a draft from a saved league, closing the gap where the same action was reachable two different ways.
- **A `fantasy-draft-app:lastLeagueId` breadcrumb** (view-location memory, not a source of truth) replaces the old single global `fantasy-draft-app:activeDraftId` key for the League path entirely — the server (`GET /drafts?league_id=`) is what actually knows whether a league has a draft, so refreshing mid-draft on draft day just needs to remember *which league* you were looking at, not *which draft*. The ad-hoc path keeps its own `activeDraftId` key (there's still only ever one ad-hoc draft in flight, unlike leagues' drafts, which are each independently resumable).

*Done*: 11 new backend tests (`list_drafts` empty/unfiltered/filtered/pick-count/round-math/complete-draft, `my_slot` range validation, league-delete cascade) + 21 new frontend tests (`DraftRoom.test.tsx` moved/extended from the old `DraftPage.test.tsx`, `LeagueDetailPage.test.tsx`, `LeaguesSection.test.tsx`, trimmed `LeaguesPage.test.tsx`/`DraftSetupForm.test.tsx`/`DraftPage.test.tsx`, updated `App.test.tsx`), all passing (216 backend / 127 frontend total), lint/typecheck/format clean on both sides.
...
**Draft view** (`app/draft_logic.py`, `app/draft.py`, `app/api/draft.py`, `Draft`/`DraftPick`/`DraftQueueEntry` models, `src/features/draft/`): the combined Phase 5 + Phase 8, plus Phase 6 live Sleeper sync. Loosely modeled on Sleeper's own draft room, per a reference screenshot: a rounds × teams board on top, a ranked/position-filterable player pool bottom-left, and Queue/Roster tabs bottom-right.

- **Setup**: a small form creates a new draft session — no `League` concept exists yet, so this is deliberately a fresh form each time rather than tied to a saved league. Two modes: **manual entry** (season/format/number of teams/number of rounds/your draft slot, all typed in) or **sync from Sleeper** (just a Sleeper draft ID/format/your draft slot — number of teams and rounds are read directly from Sleeper's own draft settings instead of retyped, so they can't drift out of sync).
- **Snake math** (`pick_to_round_and_slot`, `round_and_slot_to_pick`, `total_picks`) is pure and thoroughly tested (33 tests, including a parametrized round-trip check across many team/round counts) — `round`/`slot` are never stored on `DraftPick`, always derived from `pick_number` so there's nothing to keep in sync. Sleeper-synced picks reuse this same math (via Sleeper's own sequential `pick_no`) rather than storing Sleeper's `round`/`draft_slot` fields directly — simplest given both are standard snake order; a league using 3rd-round-reversal or linear order would need this revisited.
- **Picks are always sequential**: `POST /drafts/{id}/picks` doesn't take a pick number — it's always "the next one" — matching how an actual draft happens (one pick at a time, in order), and rejects an already-picked player or a picking-past-the-end draft (`DraftError` → HTTP 400). `DELETE` undoes the most recent pick, a safety net for mis-clicks. Both are rejected outright for a Sleeper-synced draft (`DraftError` → 400) — Sleeper is the single source of truth there, see below.
...
- **Queue**: scoped *per draft* (not global like a `RankSet`), since a draft-day shortlist is inherently tied to one specific board. Drafting a queued player automatically removes them from the queue server-side. Reordering reuses `reorderList()` via simple up/down buttons only (no full drag-and-drop here — a smaller, lower-stakes list where buttons are perfectly adequate, deliberately trading full drag support for less risk/time given Rankings already has it).
- **Roster**: a fixed starting lineup (QB/RB/RB/WR/WR/TE/FLEX/K/DEF) shown as colored position badges, each slot showing "Empty" until filled, plus a Bench section for anything drafted beyond the starters. Picks are assigned to slots greedily in draft order — the first two RBs fill the RB slots, a third goes to FLEX (if still open), everything else falls to the bench.
- **Self-healing on a stale draft**: if a draft id no longer resolves (a 404 — e.g. the draft was deleted), the shared `DraftRoom` component (below) calls an `onUnavailable` callback instead of showing a dead-end error screen; each caller recovers in its own way (the ad-hoc path clears its `localStorage` pointer, a league's detail page just drops back to its "Start Draft" control). Found this gap by literally hitting it during verification: deleted a smoke-test draft from the DB, reloaded, and got stuck on an error page with no way back — fixed before considering the phase done, and preserved through the later nesting-under-League refactor by design (see below).

*Done (Phase 5+8)*: 33 (snake math) + 14 (draft service) + 7 (draft API) new backend tests, 29 new frontend tests across setup form/board/pool/queue/roster/page-integration, all passing. Verified live end-to-end in the browser with a real mock draft: picks update the board and advance turn order correctly, undo works, queueing/drafting from the queue works, the roster groups correctly, and — critically — reloading the page mid-draft resumes exactly where it left off.

**Live Sleeper sync** (Phase 6; `app/ingest/sleeper_draft.py`, `create_sleeper_draft`/`sync_sleeper_draft`/`switch_to_manual` in `app/draft.py`): replaces manual pick entry with live polling of Sleeper's own draft, reusing the approach proven in `draft-tool/sleeper-rerun.py` (`GET /v1/draft/{id}/picks`). Since Sleeper picks already come keyed to Sleeper's own player IDs, no name-matching is needed here — reconciliation only ever mattered for Sheet ranks and ADP.

- **Setup fetches real settings**: creating a Sleeper-synced draft (`POST /drafts/sleeper`) calls `GET /v1/draft/{id}` to pull `season`/`teams`/`rounds` directly from Sleeper rather than trusting freehand input — a bad/unknown draft ID surfaces as a normal form error (`SleeperFetchError` → `DraftError` → HTTP 400) instead of creating a bogus draft.
- **Your draft slot is still typed in manually** rather than auto-detected from a Sleeper username/league lookup — deliberately simpler: you already see your own slot in Sleeper's draft room, and skipping the user→roster→slot lookup chain avoids several new failure modes (league not found, `draft_order` not yet populated pre-draft, etc.) for a personal single-user tool.
- **Polling**: the frontend calls `POST /drafts/{id}/sync` every 5 seconds while a Sleeper-synced draft is active and not yet complete (`DraftRoom`'s polling effect, shared by both the ad-hoc and league-nested paths since the nesting refactor). The backend reconciles by pick number — any pick number not already stored locally gets inserted, so re-running sync mid-draft (or after a dropped connection) is naturally idempotent; a synced player is also dropped from the local queue, same as a manual pick.
- **Manual entry is blocked, not merged**: `make_pick`/`undo_last_pick` both reject outright on a Sleeper-synced draft, so there's exactly one source of truth for picks and never a "whose version wins" conflict. A **Switch to manual** button (with a confirm prompt) flips `Draft.platform` back to `"manual"` as an escape hatch if Sleeper's API is unreachable mid-draft — manual pick entry and undo work normally from that point on, and polling stops since it's gated on `platform === "sleeper"`.
- **Verified** against the parse/reconciliation logic with fixture data (network calls are mocked at the `sleeper_draft.fetch_raw_draft`/`fetch_raw_picks` boundary in tests, per the project's "never hit live APIs in tests" rule), and separately against the real Sleeper API using a real (not-yet-started) league draft: creation pulled the actual team count/round count/season straight from Sleeper, `POST /sync` and `POST /switch-to-manual` both round-tripped correctly. This caught a real bug: Sleeper returns a genuine HTTP 404 for an unknown draft id (not a 200 with a `null` body, as first assumed), which was surfacing as an unhandled 500 instead of a clean `DraftError` → 400 until `fetch_raw_draft`/`fetch_raw_picks` were fixed to catch `httpx.HTTPStatusError` explicitly. Live-verifying a pick actually landing via polling still needs a real draft to be in progress.

*Done*: 19 new backend tests (parse/fetch, create/sync/switch-to-manual, manual-entry guards) + 8 new frontend tests, all passing (142 backend / 82 frontend total).

## Rough data model (draft)

- `League` — (platform ["sleeper" for now], platform_league_id, name, season, format, num_teams, roster_positions [JSON, Sleeper's array verbatim], team_names [JSON, {roster_id: name}], rank_set_id [nullable FK to `RankSet`], created_at) — one of your real leagues, set up once. Points at a `RankSet` via a plain reference (never the other way around — a `RankSet` has no idea which leagues use it); a `Draft` can optionally reference a `League` via `Draft.league_id`
...
- `RankSet` — (platform, season, format, name, created_at) — a named, orderable rank list; more than one can exist per (platform, season, format) (e.g. "Main" and a league-specific set sharing the same format), and `AdpEntry` stays completely separate (generic per platform/season/format, shared by every rank set/league using that format)
- `RankEntry` — (rank_set_id, platform_player_id, rank) — one player's position within a `RankSet`. No tiers yet. Built via the in-app drag-and-drop rankings builder (seeded from ADP, no `NameMapping` needed); a Sheet-based upload is a separate, still-deferred path onto this same table
...
- `Draft` — (platform ["manual" or "sleeper"], platform_draft_id [set for a Sleeper-synced draft, null for manual], league_id [nullable FK to `League`], season, format, num_teams, num_rounds, my_slot, team_names [nullable JSON, {draft_slot: name} — distinct from `League.team_names`'s roster-keyed map], created_at) — a local draft session, optionally created from a saved `League`. Deliberately does **not** store `rank_set_id`/`roster_positions` — always derived live from `Draft.league` at read time so a league's later settings changes reach existing drafts with no staleness. Deleting its `League` cascades to delete the `Draft` too (and its `DraftPick`/`DraftQueueEntry` rows) — unlike `RankSet` (which a `League` only references and can outlive), a league-created draft has no life without its league
...
- `DraftQueueEntry` — (draft_id, platform_player_id, order) — your draft-day shortlist, scoped per draft (not global like a `RankSet`)
...
Two planned paths to build your ranks, sharing one underlying rank store. In-app: pick a format, seed from ADP (or start from a previously saved set), drag-and-drop to reorder, save — done, see the "In-app rankings builder" writeup above. Sheet upload (`gspread` read of your rank Sheet, fed through the Phase 2 reconciliation pipeline) is the other, still-deferred path, for whenever there's an external rank source that isn't just "start from ADP and adjust."
...
*Done (rank-sets upgrade)*: reworked the original one-set-per-format `MyRank` table into named, multi-per-format `RankSet`/`RankEntry` tables — see the "Named, multi-per-format rank sets" writeup above. 18 backend service tests + 8 backend API tests + 13 frontend tests, all passing; verified live in the browser — creating a rank set (seeded from ADP), renaming, deleting (confirm-gated), switching between sets, and drag-reorder/save all confirmed against real data, plus confirmed the Draft tab's player pool still resolves correctly through the unchanged `GET /ranks` fallback.
...
**Phase 6 — Sleeper live sync** ✅ done
Replace manual pick entry with live polling of Sleeper's draft-picks endpoint (reusing the approach proven in `draft-tool/sleeper-rerun.py`). Note: for a Sleeper league, picks arrive already keyed by Sleeper's own player ID, so no name-matching is needed here — reconciliation only matters for Sheet ranks and ADP. See the "Live Sleeper sync" writeup above for the full breakdown.
*Done*: 19 new backend tests (parse/fetch, create/sync/switch-to-manual, manual-entry guards) + 8 new frontend tests, all passing (142 backend / 82 frontend total). See the "Live Sleeper sync" writeup above for the live-verification details, including a real bug caught and fixed against Sleeper's actual API.
...
**League setup — Phase A (RankSets)** ✅ done · **Phase B (League model + Sleeper sync)** ✅ done · **Phase C (Draft-from-League)** ✅ done
A separate initiative from the numbered phases above: set up each of your real leagues once (name/format/roster shape/team names/rank set) instead of re-entering them every draft. See the "League setup" writeup above for the full breakdown.
*Done (Phase A)*: see the "Rankings, in-app builder" entry above.
*Done (Phase B)*: 32 new backend tests (Sleeper league parsing + `app/league.py` service + API) plus 1 more in the rank-set suite (confirming `delete_rank_set` nulls out a league's reference instead of leaving it dangling) + 8 new frontend tests, all passing (172 backend / 85 frontend total). Verified live in the browser against a real Sleeper league: looked up real settings (name/team count/suggested half-PPR format from `scoring_settings.rec`), created the league, and confirmed team names/roster shape came through correctly.
*Done (Phase C)*: see the "Draft-from-League" writeup above for the full breakdown — 205 backend / 107 frontend tests total.

**Draft nested under League** ✅ done (a follow-on to League setup, not one of its numbered phases)
Once Draft-from-League existed, a top-level Draft tab sitting next to Leagues was redundant with drilling into a league directly. Draft is no longer its own tab — it lives inside a league's detail view, with the ad-hoc (non-league) path demoted to a footer link. See the "Draft nested under League" writeup above for the full breakdown.
*Done*: 216 backend / 127 frontend tests total.
