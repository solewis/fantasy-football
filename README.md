...
- `RankSet` — (platform, season, format, name, created_at) — a named, orderable rank list; more than one can exist per (platform, season, format) (e.g. "Main" and a league-specific set sharing the same format). Not tied to a `League` here — a `League` (once it exists) points at exactly one `RankSet` via a plain reference, never the other way around, and `AdpEntry` stays completely separate (generic per platform/season/format, shared by every rank set/league using that format)
