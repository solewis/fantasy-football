...
Current state: Phase 0 scaffold, Phase 1 (Sleeper player list ingestion), Phase 2 (name-matching/reconciliation tool), Phase 3's in-app path (drag-and-drop rankings builder, upgraded to named multi-per-format rank sets), Phase 4 (Sleeper ADP ingestion), the players management panel with in-UI data sync, Phases 5+8 (the draft view — board, player pool, queue, roster), and League setup Phases A+B (rank sets + the `League` model with Sleeper league sync) all done. Phase 3's Sheet-upload path, Phase 6 (live Sleeper sync), Phase 7 (availability-at-next-pick), and League setup's Phase C (wiring Draft creation to a saved League) are what's left.
...
- `Draft` — (platform ["manual" for now], platform_draft_id [null until Phase 6 sync], season, format, num_teams, num_rounds, my_slot, created_at) — a local draft session, not yet tied to a `League` (planned for League setup's Phase C)
